...
import { getAdminHistorial } from '@/lib/api';
import type { HistorialEntry, HistorialTipo } from '@/types';
import { formatCurrency } from '@/lib/utils';
import { MagnifyingGlassIcon } from '@heroicons/react/24/outline';

// ─── Tab config ──────────────────────────────────────────────────────────────
type TabKey = 'todos' | 'obligacion_agregada' | 'recarga' | 'pago_factura' | 'usuario_creado';
...
const tabs: TabItem[] = [
  ...
  { key: 'obligacion_agregada', label: 'Obligaciones agregadas', tipo: 'obligacion_agregada' },
  { key: 'recarga', label: 'Recargas', tipo: 'recarga' },
  { key: 'pago_factura', label: 'Pagos', tipo: 'pago_factura' },
  { key: 'usuario_creado', label: 'Usuarios', tipo: 'usuario_creado' },
];

// ─── Helpers ─────────────────────────────────────────────────────────────────
const tipoLabels: Record<string, string> = {
  recarga: 'Recarga',
  obligacion_agregada: 'Obligaciones agregada',
...
export default function HistorialPage() {
  ...
  // Unique users from current entries (for filter dropdown)
  ...
  const fetchHistorial = useCallback(async () => {
    ...
    const res = await getAdminHistorial({
      page,
      limit,
      tipo: tipoFilter,
      ...
      <div className="bg-gray-100 rounded-lg p-3">
        <div className="flex flex-wrap items-center gap-3 justify-between">
          ...
          <select
            ...
            className="px-4 py-2 bg-white border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-orange-500 flex items-center gap-2"
          >
